...
var x=Object.defineProperty;var P=Object.getOwnPropertyDescriptor;var I=Object.getOwnPropertyNames;var T=Object.prototype.hasOwnProperty;var j=(o,i)=>{for(var t in i)x(o,t,{get:i[t],enumerable:!0})},A=(o,i,t,e)=>{if(i&&typeof i=="object"||typeof i=="function")for(let n of I(i))!T.call(o,n)&&n!==t&&x(o,n,{get:()=>i[n],enumerable:!(e=P(i,n))||e.enumerable});return o};var E=o=>A(x({},"__esModule",{value:!0}),o);var z={};j(z,{default:()=>v});module.exports=E(z);var c=require("obsidian");var w=require("obsidian"),p=class extends w.PluginSettingTab{constructor(t,e){super(t,e);this.plugin=e}display(){let{containerEl:t}=this;t.empty(),t.createEl("h2",{text:"Settings"}),new w.Setting(t).setName("Anthropic API Key").setDesc("Won`t leave your vault").addText(e=>e.setPlaceholder("Enter your key").setValue(this.plugin.settings.anthropicKey).onChange(async n=>{this.plugin.settings.anthropicKey=n,await this.plugin.saveSettings()}))}};var F={anthropicKey:""};var C=require("obsidian");var m={generate_dictionary_entry:`You are an expert linguist specializing in the German language. Your task is to create a detailed dictionary entry for a given German word. Here's the word you need to analyze:
...
<ideal_output>
\u{1F624} [[verfeinden]], [f\u025B\u0250\u032F\u02C8fa\u026A\u032Fnd\u0259n] | [[verfeindete]], haben [[verfeindet]]
...
</example>`};var b=class{constructor(i){this.settings=i}async fetchTemplate(i){return this.makeRequest(i,m.generate_dictionary_entry)}async determineInfinitiveAndEmoji(i){return this.makeRequest(i,m.determine_infinitive_and_pick_emoji)}async makeBrackets(i){return this.makeRequest(i,m.make_brackets)}async translateText(i){return this.makeRequest(i,m.translate_text)}async makeRequest(i,t){let e="https://api.anthropic.com/v1/messages",n={"Content-Type":"application/json","x-api-key":this.settings.anthropicKey,"anthropic-version":"2023-06-01"},a={model:"claude-3-5-haiku-20241022",max_tokens:1024,system:[{type:"text",text:t,cache_control:{type:"ephemeral"}}],messages:[{role:"user",content:i}]};try{let r=await(0,C.requestUrl)({url:e,method:"POST",contentType:"application/json",body:JSON.stringify(a),headers:n});return JSON.stringify(r)}catch(r){return r+`

`+JSON.stringify({url:e,method:"POST",headers:n,body:a})}}};var f=require("obsidian"),k=class{constructor(i){this.vault=i}async appendToFile(i,t){let e=this.vault.getAbstractFileByPath(i);if(!e){try{await this.vault.create(i,t)}catch(n){new f.Notice(`Error creating file: ${n.message}`)}return}if(!(e instanceof f.TFile)){new f.Notice(`File ${i} does not exist or is not a valid file!`);return}try{let a=await this.vault.read(e)+t;await this.vault.modify(e,a)}catch(n){new f.Notice(`Error appending to file: ${n.message}`)}}async doesFileContainContent(i,t){let e=this.vault.getAbstractFileByPath(i);return!e||!(e instanceof f.TFile)?null:(await this.vault.read(e)).includes(t)}};var v=class extends c.Plugin{async onload(){await this.loadSettings(),this.apiService=new b(this.settings),this.fileService=new k(this.app.vault),this.addCommand({id:"backlink-all-to-current-file",name:"Add backlinks to the current file in all referenced files",editorCallback:async(t,e)=>{var S,_;let n=(S=e.file)==null?void 0:S.name;if(!e.file||!n){new c.Notice("Current file is missing a title");return}let{metadataCache:a,vault:r}=this.app,s=a.getFileCache(e.file),d=(_=s==null?void 0:s.links)!=null?_:[],l=[];for(let h of d){let u=h.link,g=a.getFirstLinkpathDest(u,e.file.path);g instanceof c.TFile?l.push({name:u,path:g.path}):l.push({name:u,path:null})}for(let h of l)try{let u,g=`[[${n.split(".")[0]}]]`;if(h.path)u=h.path;else{let y=`Worter/${h.name[0].toUpperCase()}`;r.getAbstractFileByPath(y)||await r.createFolder(y),u=`${y}/${h.name}.md`}await this.fileService.doesFileContainContent(u,g)||await this.fileService.appendToFile(u,`, ${g}`)}catch(u){new c.Notice(`Error processing link ${h.name}: ${u.message}`)}}}),this.addCommand({id:"fill-template",name:"Fill the template for the word in the title of the file",editorCallback:async(t,e)=>{var s,d;let n=(s=e.file)==null?void 0:s.name;if(!n){new c.Notice("Current file is missing a title");return}let a=await this.apiService.fetchTemplate(n),r=this.extractContentFromResponse(a);r&&((d=e==null?void 0:e.file)!=null&&d.path)&&await this.fileService.appendToFile(e.file.path,r)}}),this.addCommand({id:"get-infinitive-and-emoji",name:"Get infinitive form and emoji for current word",editorCallback:async(t,e)=>{var s,d;let n=(s=e.file)==null?void 0:s.name;if(!n){new c.Notice("Current file is missing a title");return}let a=await this.apiService.determineInfinitiveAndEmoji(n),r=this.extractContentFromResponse(a);r&&((d=e==null?void 0:e.file)!=null&&d.path)&&await this.fileService.appendToFile(e.file.path,r)}}),this.addCommand({id:"duplicate-selection",name:"Duplicate selected text and process with brackets",editorCallback:async t=>{let e=t.getSelection();if(e){let n=t.getCursor(),a=await this.apiService.makeBrackets(e),r=this.extractContentFromResponse(a);r&&(t.replaceSelection(e+`
...
`+r+`
`),t.setCursor({line:n.line,ch:n.ch+e.length}))}}}),this.addSettingTab(new p(this.app,this))}async loadSettings(){this.settings=Object.assign({},F,await this.loadData())}async saveSettings(){await this.saveData(this.settings)}extractContentFromResponse(t){var e,n,a,r,s,d;try{let l=JSON.parse(t);return((a=(n=(e=l==null?void 0:l.json)==null?void 0:e.content)==null?void 0:n[0])==null?void 0:a.text)||((d=(s=(r=l==null?void 0:l.json)==null?void 0:r.content)==null?void 0:s[0])==null?void 0:d.text)||""}catch(l){return""}}};
